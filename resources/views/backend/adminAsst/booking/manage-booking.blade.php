...
<section class="booking_table_area" style="height: 220%;">
    <div class="container">
        <div class="s_black_title" style="margin-top: 5rem; margin-bottom: 5rem;">
            ...
            <form action="{{url('/adminAsst/search-reserveTableDate')}}" enctype="multipart/form-data" method="post">@csrf
                <div class="row">
                    <div class="col-sm-3">
        ...
        <form action="{{url('/adminAsst/create-booking')}}" enctype="multipart/form-data" method="post">@csrf
            <div class="row">
                <div class="col-sm-3">
            ...
            @foreach ($tables as $table)
                ...
                <div class="col-sm-3">
                    @if($table->status == 'ว่าง')
                        <a href="{{url('/adminAsst/edit-booking')}}/{{$table->id}}" class="btn btn-default submit_btn_success">โต๊ะ {{$number}} จำนวน {{$table->seat}} ท่าน</a>
                    @else 
                        <a href="{{url('/adminAsst/edit-booking')}}/{{$table->id}}" class="btn btn-default submit_btn2">โต๊ะ {{$number}} จำนวน {{$table->seat}} ท่าน</a>
                    @endif
                </div> 
            @endforeach
        </div>
